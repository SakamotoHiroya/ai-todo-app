...
// ToggleTaskCompletionParams is parameters of toggleTaskCompletion operation.
type ToggleTaskCompletionParams struct {
	ID string
}

func unpackToggleTaskCompletionParams(packed middleware.Parameters) (params ToggleTaskCompletionParams) {
	{
		key := middleware.ParameterKey{
			Name: "id",
			In:   "path",
...
func decodeToggleTaskCompletionParams(args [1]string, argsEscaped bool, r *http.Request) (params ToggleTaskCompletionParams, _ error) {
	// Decode path: id.
	if err := func() error {
		param := args[0]
		if argsEscaped {
